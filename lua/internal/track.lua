...
local function apply_config(config)
	...
	map("o", config.mappings.textobject, "<Cmd>lua MiniComment.textobject()<CR>", { desc = "Comment textobject" })
end
...
H.make_comment_parts = function(ref_position)
	...
	local cs = call_safely(options.custom_commentstring, ref_position) or MiniComment.get_commentstring(ref_position)

	if not cs or #cs == 0 then
		api.nvim_echo({ { "(mini.comment) ", "WarningMsg" }, { [[Option 'commentstring' is empty.]] } }, true, {})
		return { left = "", right = "" }
	end
